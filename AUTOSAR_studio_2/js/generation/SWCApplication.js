...
function Generate_Swc_Arxml() {
	...
	console.log(SWCsTypeArray);
	for (var i = 0; i < SWCsTypeArray.length; i++) {
		if (SWCsTypeArray[i].name == softwareComponentName) {
			switch (SWCsTypeArray[i].type) {
				case "Application":
					XML.BeginNode("APPLICATION-SW-COMPONENT-TYPE");
					break;
				case "Complex":
					XML.BeginNode("COMPLEX-DEVICE-DRIVER-SW-COMPONENT-TYPE");
					break;
				case "Service":
					XML.BeginNode("SERVICE-SW-COMPONENT-TYPE");
					break;
			}
		}
	}
